...
public:
    ...
    vector<Document> AddFindRequest(const string& raw_query, DocumentStatus status) {
        vector<Document> v = search_server_.FindTopDocuments(raw_query, status);
        QueryResult q;
        q.is_doc = v.size() > 0;
        requests_.push_back(q);
        CheckRequest();
    ...
    vector<Document> AddFindRequest(const string& raw_query) {
        vector<Document> v = search_server_.FindTopDocuments(raw_query);
...
ostream& operator<<(ostream& out, const Document& doc){
    out << "{ document_id = " << doc.id << ", relevance = " << doc.relevance << ", rating = " << doc.rating << " }"s;
    return out; 
}

template <typename DocumentIterator>
ostream& operator<<(ostream& out, const IteratorRange<DocumentIterator> range){
